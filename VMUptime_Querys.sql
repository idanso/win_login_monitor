...
mod_reservations.Active AS 'Status',
  CONCAT(
   ...
   CONCAT(ROUND(((UPTime_Logs.sum_time_deff/differences.seconds)*100), 1),'%') AS 'Active (%)'
, CONCAT(ROUND(((Logins_logs.sum_time_deff/UPTime_Logs.sum_time_deff)*100), 1),'%') AS 'Login/Active (%)'
FROM (SELECT t.VMName,SUM(t.Time_Deff) AS 'sum_time_deff'
		FROM (SELECT t.VMName,
